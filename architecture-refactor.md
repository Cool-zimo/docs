...
### 面包屑：原来就有，问题是溢出方式是横向滚动

深路径下 CSS `max-width:45%` + `overflow-x:auto`，
**当前目录名经常被滚出视野** —— 这才是"感觉没有面包屑"的主因。

现在：超过 5 段折叠中间层（根 + 末尾若干级），
单段超 24 字符截断保留后缀，根显示"网盘"。

★ 契约与 `textutil.py` 完全一致
  （`BREADCRUMB_ROOT_LABEL` / `MAX_CRUMBS` / `ELLIPSIS` / `shorten`）

★ 折叠项 `path` 为 `null`，渲染必须判空 ——
  否则 `data-path` 变成字符串 `"null"`，点击跳到不存在的路径。

### 维护面板：js/maintain.js

`scan()` 扫所有仓库 tree 比对 VFS；`buildPlan/applyPlan`
只增不删、重名加序号、绝不覆盖现有文件。

★ 仓库扫失败时**不判幽灵** —— 没扫到不等于不存在。
  否则一次网络抖动就会把正常文件标成"已丢失"。

★ 入口放设置而不是工具栏：维护动作，不是日常操作。

### ⚠️ 又犯了同一个错：release 停在草稿态

`workflow` 里 `draft: true` 导致 **v0.0.8 / 0.0.9 / 0.0.10 / 0.0.11
连续四个版本**构建全部成功，但主页看不到、`/releases/latest` 404。

更糟的是我上一轮亲口说「已给 workflow 加了自动发布」——
**实际根本没改那一行**。这次改的是源头（`draft: false`），
并手动补发了四个版本。

> 教训：声称"已修复"之前，去读那一行代码。
> 这是第 5 次同类错误（仓鼠文档 / 嵌套目录 / OAuth 章节 /
> config 字段名 / draft 状态），共同点都是**没核实就汇报**。

### 测试

js 新增 47 项（维护 31 + 面包屑 16）。
★ 面包屑测试用 `vm` 加载**真实 class**，不是复刻方法体 ——
  复刻的话改了源码测试照样绿，等于没测。`
